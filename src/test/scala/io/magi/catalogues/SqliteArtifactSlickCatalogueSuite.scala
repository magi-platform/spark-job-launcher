...
class SqliteArtifactSlickCatalogueSuite extends SqliteSlickTestBase[ Artifact ] with BeforeAndAfterAll with BeforeAndAfter {
    ...
    "Artifact Catalogue" should "save a new artifact" in {
        val created : Artifact = Await.result( catalogue.create( Artifact( 0, s"$unique()", "1", "Main", "asdf" ) ), awaitDuration )
    ...
    "Artifact Catalogue" should "update an existing artifact" in {
        val created = Await.result( catalogue.create( Artifact( 0, s"$unique()", "1", "Main", "asdf" ) ), awaitDuration )
